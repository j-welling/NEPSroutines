...
Before copying the relevant files `NEPSroutines::Setup()` will ask for some information in the R console about the NEPS study and the competence domain. This information is used to set the file name for the Quarto template and configure its content for the specific domain (e.g., refer to the correct domain within the technical report). Currently, the function is only designed for reading, mathematical, and digital competence. For other domains, the template for reading competence should be used and adapted accordingly.
