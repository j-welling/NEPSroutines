...
linking <- function(resp_curr,
    ...
    if (save) {
        ...
        if (do_dif) {
            ...
            name <- create_name("linking", name_group, ".xlsx")
            save_table(
        ...
        name <- create_name("linking", name_group, ".rds")
        save_results(linking, filename = name, path = path_results)
    }
...
prepare_longitudinal_resp <- function(resp_curr,
    ...
    check_variables(resp_curr, "resp_curr", "ID_t")
    check_variables(resp_prev, "resp_prev", "ID_t")
    if (is_anchor_group)
        check_variables(resp_link, "resp_link", "ID_t")
    ...
    check_numerics(resp_curr, "resp_curr", items_curr, check_invalid = TRUE)

    items_prev <- vars_prev$item[vars_prev[[select_prev]]]
    check_numerics(resp_prev, "resp_prev", items_prev, check_invalid = TRUE)

    if (is_anchor_group) {
        items_link <- vars_link$item[vars_link[[select_link]]]
        check_numerics(resp_link, "resp_link", items_link, check_invalid = TRUE)
    } else {
        items_link <- NULL
    }
...
test_linking_data <- function(vars_curr,
    ...
    check_logicals(vars_curr, "vars_curr", select_curr, warn = warn)
    check_items(vars_curr$item[vars_curr[[select_curr]]])
    check_logicals(vars_prev, "vars_prev", select_prev, warn = warn)
    check_items(vars_prev$item[vars_prev[[select_prev]]])
    if (!is.null(select_link) & !is.null(vars_link))
        check_logicals(vars_link, "vars_link", select_link, warn = warn)
    check_items(vars_link$item[vars_link[[select_link]]])

    # Test items
    if (!is.null(resp_curr))
      check_numerics(resp_curr, "resp_curr", vars_curr$item[vars_curr[[select_curr]]])
    if (!is.null(resp_prev))
      check_numerics(resp_prev, "resp_prev", vars_prev$item[vars_prev[[select_prev]]])
    if (!is.null(resp_link) & !is.null(vars_link) & !is.null(select_link))
      check_numerics(resp_link, "resp_link", vars_link$item[vars_link[[select_link]]])

    # Test valid
    if (!is.null(valid_curr) & !is.null(resp_curr))
        check_logicals(resp_curr, "resp_curr", valid_curr, warn = warn)
    if (!is.null(valid_prev) & !is.null(resp_prev))
        check_logicals(resp_prev, "resp_prev", valid_prev, warn = warn)
    if (!is.null(valid_link) & !is.null(resp_link))
        check_logicals(resp_link, "resp_link", valid_link, warn = warn)

    # Test scoring
    if (!is.null(scoring_curr))
        check_numerics(vars_curr, "vars_curr", scoring_curr, check_invalid = TRUE)
    if (!is.null(scoring_prev))
        check_numerics(vars_prev, "vars_prev", scoring_prev, check_invalid = TRUE)
    if (!is.null(scoring_link) & !is.null(vars_link))
        check_numerics(vars_link, "vars_link", scoring_link, check_invalid = TRUE)

    # Test pweights
    if (!is.null(pweights_curr) & !is.null(resp_curr))
        check_numerics(resp_curr, "resp_curr", pweights_curr, check_invalid = TRUE)
    if (!is.null(pweights_prev) & !is.null(resp_prev))
        check_numerics(resp_prev, "resp_prev", pweights_prev, check_invalid = TRUE)
    if (!is.null(pweights_link) & !is.null(resp_link))
        check_numerics(resp_link, "resp_link", pweights_link, check_invalid = TRUE)
    ...
    # Test mvs
    if (warn) is_null_mvs_valid(mvs = mvs)
