...
pc_scoring <- function(resp, poly_items, vars = NULL, select = NULL,
                       ...
                       path_results = "Results", save = TRUE,
                       verbose = TRUE) {
  ...
  if (impute) {
    ...
    if (verbose) {
      message( "Missing values of subitems are imputed if the share of ",
               "missing responses falls below the 'threshold'. ",
               "However, the returned dataset remains unchanged, that is, ",
               "it includes the original (non-imputed) responses for the ",
               "subitems.\n",
               "To skip subitem imputation, set 'imputation = FALSE'." )
    }
...
pc_missing_subitems <- function( resp, mvs, poly_items,
                                 missing_by_design = -54, threshold,
                                 path_results, save ) {
  ...
  NEPSroutines:::check_numerics(resp, "resp", unlist(poly_items), dich = TRUE)
  if ( !is.numeric(threshold) | threshold < 0 | threshold > 1) {
    stop( "The argument 'treshold' must be numeric in the interval between ",
          "0 and 1. Please check your input." )
  }
...
pc_imputation <- function( resp, vars, select,
  ...
  if ( is.null(indicators) | !is.data.frame(indicators) ) {
    stop( "The imputation of missing values on subitems, requires a ",
          ...
          "using 'pc_scoring()' with 'imputation = TRUE'. ",
          "Please contact the package developers." )
  }
  ...
  # Save results
  if (save) {
    subitems_imputation_details <- list(
      fit = fit,
      pred_resp = pred_resp,
