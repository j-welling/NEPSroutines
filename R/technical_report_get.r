...
#' (`=`, `==`, `!=`, `<`, `<=`, `>`, `>=`; use `>=` / `<=`, not the reversed
#' `=>` / `=<`) with a number of missing values; e.g.,
#'
#' * `"=3"` returns the percentage for 3 missing values
#' * `"<3"` returns the percentage for less than 3 missing values
#' * `">=3"` returns the percentage for 3 or more missing values
#'
#' A number without an operator (e.g. `3`) is treated as `=3`. Multiple
#' conditions can be combined within the string using `&` and `|`
#' (e.g. `">2 & <5"`); surrounding whitespace is ignored.
#' @param digits A number for rounding.
...
#' @param dif A function used to summarize the DIF effects, or a single
#' string giving a comparison operator (`=`, `==`, `!=`, `<`, `<=`, `>`,
#' `>=`) followed by a number. Use `>=` / `<=`, not the reversed `=>` / `=<`.
#' For example:
#'
#' * `"=1"` returns the number of DIF effects equal to 1
#' * `"<1"` returns the number of DIF effects less than 1
#' * `">=1"` returns the number of DIF effects greater than or equal to 1
#'
#' Multiple conditions can be combined within the string using `&` and `|`
#' (e.g. `">.4 | <.05"`); surrounding whitespace is ignored. If the DIF
#' variable has more than two groups and `group` is missing, results across
